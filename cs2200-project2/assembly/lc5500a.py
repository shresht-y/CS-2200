...
"""lc5500a.py: A definition of the RAMA-2200 architecture."""
...
class push(Instruction):
    __RE_PUSH = re.compile(r'^\s*(?P<SR>\$\w+?)\s*$')
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        match = cls.__RE_PUSH.match(operands)

        if match is None:
            raise RuntimeError(
                "Operands '{}' are in an incorrect format.".format(operands.strip()))

        result_list.append(parse_register(match.group('SR')))
        result_list.append('1101')
        result_list.append('1' * IMM_OFFSET_SIZE)
...
class pop(Instruction):
    __RE_PUSH = re.compile(r'^\s*(?P<DR>\$\w+?)\s*$')
    # re.compile(r'^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*(,\s*(?P<ShfType>LSL|LSR|ASR)\s+(?P<ShfDist>\d+))?$', re.I)
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        result_list.append('1101')
        result_list.append('0' * (IMM_OFFSET_SIZE - 1))
