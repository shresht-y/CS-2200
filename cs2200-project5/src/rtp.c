...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    if (length == MAX_PAYLOAD_LENGTH) {
        ...
        amount_of_packets = length / MAX_PAYLOAD_LENGTH;
    ...
    packets = malloc(sizeof(packet_t) * (size_t) amount_of_packets);
...
static void *rtp_recv_thread(void *void_ptr) {
    ...
    do {
        ...
        do {
            ...
            *     received.
            */
           if (packet.type == NACK || packet.type == ACK) {
...
static void *rtp_send_thread(void *void_ptr) {
    ...
    do {
        ...
        for (i = 0; i < array_length; i++) {
            ...
            //must reset the connection to say that the connection has not recieved a packet
            connection->recieved=0;
