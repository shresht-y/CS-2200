...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        pfn_t output = NUM_FRAMES;
        uint8_t comparison = 255;
        for (pfn_t i = 0; i < num_entries; i++) {
            if (!frame_table[i].protected) {
                if (frame_table[i].ref_count < comparison) {
                    comparison = frame_table[i].ref_count;
                    output = i;
                }
            }
        }
        ...
        //use i % num entries as the index because we must treat this as a circular array
        //pfn_t unprotected_found = NUM_FRAMES;
        for (pfn_t i = last_evicted; i<(num_entries + last_evicted)*2; i++) {
            if (!frame_table[i % num_entries].protected) {
                if (frame_table[i % num_entries].referenced != 1) {
                    //unprotected_found = i;
                    last_evicted = (i+1) % num_entries;
                    return i % num_entries;
                }
                //clear the reference bit as we go
...
void daemon_update(void) {
    /** FIX ME */
    //we need to push the reference bit MSB (bit 8) to the reference count for eacch 
    size_t num_entries = MEM_SIZE / PAGE_SIZE;
    for (pfn_t i = 0; i < num_entries; i++) {
        //we want to push the reference bit 
        if (!frame_table[i].protected) {
            //grab the reference value and left shift by the size of int (i) 10000000
            uint8_t ref = frame_table[i].referenced << 7;
            //right shift the ref count
            frame_table[i].ref_count = frame_table[i].ref_count >> 1; //10000000 -> 01000000
            frame_table[i].ref_count += ref;
